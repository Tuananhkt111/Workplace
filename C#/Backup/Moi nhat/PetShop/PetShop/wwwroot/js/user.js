...
function searchTran() {
    $.ajax({
        type: "POST",
        url: "/Transaction/FindTranByUsername",
        dataType: 'json',
        ...
        error: function () {
            alert("/Transaction/FindTranByUsername error");
        }
    });
}
...
function loadProfile() {
    $.ajax({
        type: "POST",
        url: "/Principal/LoadProfile",
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify(usernameValue),
        success: function (data) {
            $('#txtPasswordUpdt').val(data.password);
            $('#txtFullnameUpd').val(data.fullname);
            $('#txtPhoneUpdt').val(data.phone);
            $('#txtAddressUpdt').val(data.address);
        },
        error: function () {
            alert("Principal/LoadProfile error");
        }
    });
}
...
function loadFavoriteList() {
    if (roleValue === "user") {
        if (listAccessoryFavorite.length !== 0) {
            $('#favoritelist').html("");
            ...
            x.innerHTML = "No favorite available";
            $('#favoritelist').append(x);
        }
    } else {
...
function updateProfile() {
    var jsonObj = {
        ...
        Fullname: $('#txtFullnameUpd').val(),
        Phone: $('#txtPhoneUpdt').val(),
        Address: $('#txtAddressUpdt').val()
    }
    $.ajax({
        type: "POST",
        url: "/Principal/UpdateProfile",
        dataType: 'json',
        contentType: "application/json; charset=utf-8",
        ...
        error: function () {
            alert("Principal/UpdateProfile error.");
        }
    });
}
$(function () {
