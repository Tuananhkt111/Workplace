 ...
 * @author tuana
 */
public class SAXController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            StudentHandler sh = new StudentHandler(username, pass);
            XMLUtils.parseFileWithSAX(filePath, sh);
            if (sh.isFound()) {
                HttpSession session = request.getSession();
                session.setAttribute("FULLNAME", sh.getFullname());
                url = SUCCESS;
            }
        } catch (Exception e) {
            e.printStackTrace();
